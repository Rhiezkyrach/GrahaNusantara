...
class FokusController extends Controller
{
    ...
    public function fokus(Fokus $fokus, Request $request)
    {
        $terkiniKanal =  Berita::kategoriTerkini($this->current_network)->where('id_fokus', $fokus->id)->paginate(20);

        //Category Ajax
        if ($request->ajax()) {
            return view('layouts.partials.fokus_ajax', [
        ...
        return view('fokus', [
            'network_utama' => $this->network_utama,
            'judul' => 'Berita ' . $fokus->nama . ' - ' .  $this->current_setting->judul_situs,
            'deskripsi' => 'Berita ' . $fokus->nama . ' terkini',
            'keyword' => 'Berita ' . $fokus->nama . ' terkini',
            ...
            'hlkategori' => Berita::kategoriTerkini($this->current_network)->where('id_fokus', $fokus->id)->where('headline', 1)->limit(5)->get(),
