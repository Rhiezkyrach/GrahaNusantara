...
<div class="block w-full mb-8">
  <form method="post" action="/indeks">
      ...
      <div class="flex flex-col md:flex-row gap-2 w-full md:items-center">
          ...
          <input type="date" name="tanggal" class="px-4 py-2 w-full text-sm md:w-44 border border-gray-400 rounded-lg active:outline-none" value="{{ old('tanggal', $tanggal) }}">
...
<div class="font-semibold mb-5 dark:text-gray-100">BERITA {{ strToUpper(Carbon\Carbon::parse($tanggal)->translatedFormat('l, d F Y')) }} </div>
@if($indeks->count())
