...
<!-- main Container -->
<x-main-container title="Edit Berita" url="{{ $setting && $setting->Network ? $setting->Network->url : url('/') }}">
  ...
  <div class="flex flex-col mt-5 bg-gray-200 p-4 h-auto w-full rounded-lg">
    ...
    @if($galeri->count())
      <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
        <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">GAMBAR SLIDE</span></div>
          <div class="images-preview grid grid-cols-3 md:grid-cols-5 gap-2 overflow-hidden">
            @foreach($galeri as $g)
              <div class="w-full h-32">
                <form action="/admin/galeri/{{ $g->id }}" method="post">
                  @method('delete')
                  @csrf
                  <button class="hapusFotoGaleri absolute text-xs text-white bg-red-600 hover:bg-red-700 p-2 rounded-sm"><i class="fas fa-trash-alt"></i></button>
                </form>
                <img src="{{ asset('storage/'. $g->nama_photo) }}" class="mb-2 w-full h-full object-cover rounded-md overflow-hidden">
              </div>
            @endforeach
        </div>

        {{-- INPUT GAMBAR SLIDE --}}
        <div class="mt-2 relative bg-gradient-to-b from-slate-300 rounded-lg">
            <label class="mb-1" for="inputgambar_detail"><span class="text-xs uppercase py-1 pl-2.5 font-semibold">GAMBAR SLIDE</span></label>
            <input type="file" form="editform" id="nama_photo" name="nama_photo[]" class="w-full file-input file-input-bordered" required multiple>
            @error('nama_photo')
    ...
    <form id="editform" method="post" action="/admin/berita/{{ $berita->slug }}" enctype="multipart/form-data">
    @csrf
    @method('put')
    
      <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
        <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">EDIT BERITA</span></div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-2.5">
          ...
          <div class="bg-gradient-to-b from-slate-300 rounded-lg">
              ...
              <select id="inputid_kategori" name="id_kategori" class="select2 main_input_md" required>
                ...
                @foreach($kategori as $kt)
                  <option value="{{ $kt->id }}" @selected( $berita->id_kategori == $kt->id)>{{ $kt->nama }}</option>
                @endforeach
              </select>
          ...
          <div class="bg-gradient-to-b from-slate-300 rounded-lg">
              ...
              <select id="inputid_fokus" name="id_fokus" class="select2 main_input_md">
                <option class="text-gray-500 "value=""></option>
                @foreach($fokus as $f)
                  <option value="{{ $f->id }}" @selected( $berita->id_fokus == $f->id)>{{ $f->nama }}</option>
                @endforeach
              </select>
          ...
          <div class="md:col-span-2 bg-gradient-to-b from-slate-300 rounded-lg">
            ...
            <input type="text" id="inputjudul_atas" name="judul_atas" class="main_input_md" placeholder="Judul Atas" value="{{ old('judul_atas', $berita->judul_atas) }}" autocomplete="off">
          ...
          <div class="md:col-span-4 bg-gradient-to-b from-slate-300 rounded-lg">
            ...
            <input type="text" id="inputjudul" name="judul" class="main_input_md" placeholder="Judul (Panjang Maks. 80 Karakter)" value="{{ old('judul', $berita->judul) }}" autocomplete="off" required>
          ...
          <div class="md:col-span-4 bg-gradient-to-b from-slate-300 rounded-lg">
            ...
            <input type="text" id="inputsub_judul" name="sub_judul" class="main_input_md" placeholder="Sub Judul" value="{{ old('sub_judul', $berita->sub_judul) }}" autocomplete="off">
          ...
          <div class="md:col-span-4 bg-gradient-to-b from-slate-300 rounded-lg">
            ...
            <textarea id="isi" name="isi" required>{{ $berita->isi }}</textarea>
          ...
          <div class="md:col-span-4 bg-gradient-to-b from-slate-300 rounded-lg">
            ...
            <input type="text" id="inputtag" name="tag" class="main_input_md" placeholder="Tag (pisahkan dengan koma)" value="{{ old('tag', $berita->tag) }}" autocomplete="off" required>
      ...
      <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
        <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">GAMBAR</span></div>
          ...
          <div class="grid grid-cols-1 gap-2.5">
            ...
            <div class="relative bg-gradient-to-b from-slate-300 rounded-lg">
              <label class="mb-1" for="inputgambar_detail"><span class="text-xs uppercase py-1 pl-2.5 font-semibold">GAMBAR</span></label>
                @if($berita->gambar_detail)
                  <input name="gambarLama" type="file" class="hidden" value="{{ $berita->gambar_detail }}">
                  <img class="img-preview mb-2 w-full h-60 object-cover overflow-hidden" src="{{ asset('storage/'. $berita->gambar_detail) }}">
                @else
            ...
            <div class="bg-gradient-to-b from-slate-300 rounded-lg">
              <label class="mb-1" for="inputcaption"><span class="text-xs uppercase py-1 pl-2.5 font-semibold">CAPTION</span></label>
                <textarea id="inputcaption" name="caption" class="main_input_md" placeholder="Caption Gambar" rows="5" autocomplete="off" required>{{ old('caption', $berita->caption) }}</textarea>
        ...
        <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
          <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">WARTAWAN</span></div>
          ...
          <div class="grid grid-cols-1 gap-2.5">
            ...
            <div class="bg-gradient-to-b from-slate-300 rounded-lg">
              ...
              <select id="inputid_wartawan" name="id_wartawan" class="select2 main_input_md" required>
                ...
                <option value="{{ $w->id_wartawan }}" @selected($berita->id_wartawan == $w->id_wartawan)>{{ $w->nama_wartawan }}</option>
                @endforeach
              </select>
              @endif
            ...
            <div class="bg-gradient-to-b from-slate-300 rounded-lg">
              ...
              <input type="text" id="inputpenulis" name="penulis" class="main_input_md" placeholder="{{ auth()->user()->name }}" value="{{ old('penulis', $berita->penulis) }}" autocomplete="off" readonly>
              ...
              <div class="relative my-1 flex w-full">
                <div class="mx-auto w-32 h-32 rounded-full overflow-hidden border-2 border-gray-400">
                  @if($berita->foto_penulis)
                    <input name="fotoPenulisLama" type="file" class="hidden" value="{{ $berita->foto_penulis }}">
                    <img class="penulis-preview mb-2 w-full h-full object-cover" src="{{ asset('storage/'. $berita->foto_penulis) }}">
                  @else
              ...
              {{-- <div class="col-span-2 grid grid-cols-1 gap-2.5"> --}}
                ...
                <div class="bg-gradient-to-b from-slate-300 rounded-lg">
                  <label class="mb-1" for="inputoleh"><span class="text-xs uppercase py-1 pl-2.5 font-semibold">OLEH</span></label>
                    <input type="text" id="inputoleh" name="oleh" class="main_input_md" placeholder="Penulis" value="{{ old('oleh', $berita->oleh) }}" autocomplete="off">
        ...
        <div class="grid grid-cols-1 gap-2.5">
          ...
          <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
            <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">EMBED</span></div>
            ...
            <div class="grid grid-cols-1 gap-2.5">
              ...
              <div class="bg-gradient-to-b from-slate-300 rounded-lg">
                ...
                <input type="text" id="inputvideo" name="video" class="main_input_md" placeholder="URL Video" value="{{ old('video', $berita->video) }}" autocomplete="off">
              ...
              <div class="bg-gradient-to-b from-slate-300 rounded-lg">
                ...
                <input type="text" id="kode_embed" name="kode_embed" class="main_input_md" placeholder="Embed Instagram/Twitter/Youtube" value="{{ old('kode_embed', $berita->kode_embed) }}" autocomplete="off">
          ...
          <div class="w-full h-auto px-2 py-2 border border-amber-500 rounded-md itemrow bg-amber-100/25">
            <div class="-mt-1.5 mb-2.5 text-center font-semibold"><span class="px-4 py-1 rounded-b-lg bg-amber-500 text-white">PUBLISH</span></div>
            ...
            <div class="grid grid-cols-1 gap-2.5">
              ...
              <div class="bg-gradient-to-b from-slate-300 rounded-lg">
                ...
                <input type="date" id="inputtanggal_tayang" name="tanggal_tayang" class="main_input_md" value="{{ old('tanggal_tayang', Carbon\Carbon::parse($berita->tanggal_tayang)->translatedFormat('Y-m-d')) }}" autocomplete="off" required>
              ...
              <div class="bg-gradient-to-b from-slate-300 rounded-lg">
                ...
                <input type="time" id="inputwaktu" name="waktu" class="main_input_md" value="{{ old('waktu', Carbon\Carbon::parse($berita->waktu)->translatedFormat('H:i')) }}" autocomplete="off" required>
              ...
              <div class="form-control bg-slate-300 px-2 rounded">
                <label class="label cursor-pointer">
                  ...
                  <input type="checkbox" id="headline" name="headline" value="1" class="checkbox checkbox-primary" {{ $berita->headline == '1' ? 'checked' : '' }}/>
              ...
              <div class="form-control bg-slate-300 px-2 rounded">
                <label class="label cursor-pointer">
                  ...
                  <input type="checkbox" id="publish" name="publish" value="1" class="checkbox checkbox-primary" {{ $berita->publish == '1' ? 'checked' : '' }} @if(auth()->user()->level == 'wartawan') disabled @endif />
...
$(document).ready(function () {
    ...
    var previewImages = function(input, imgPreviewPlaceholder) {
      if (input.files) {
        ...
        for (i = 0; i < filesAmount; i++) {
          ...
          reader.onload = function(event) {
            $($.parseHTML('<img>')).attr('src', event.target.result).attr('class', 'w-full h-32 object-cover rounded-md').appendTo(imgPreviewPlaceholder);
          }
  ...
  // Hapus Foto Galeri
  $('.hapusFotoGaleri').on('click', function(){
    event.preventDefault();
      let id = $(this).data('id');
      Swal.fire({
          title: 'Hapus Foto?',
