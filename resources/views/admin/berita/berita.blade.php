...
<div class="w-full h-auto bg-white px-5 my-20 md:mx-8 md:mt-5 md:mb-14">
  <div class="flex flex-row items-center justify-between">
    <div class="md:text-2xl font-semibold">List Berita</div>
  ...
  {{-- main content --}}
    ...
    <div class="mt-5 h-auto w-full">
        <div class="flex flex-row w-full justify-between">
            <a href="/admin/berita/create"><div class="text-white font-semibold bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg"><i class="fas fa-plus"></i> Tambah</div></a> 
            <form action="/admin/berita" autocomplete="off">
              ...
              <table class="min-w-full">
                ...
                <tbody>
                  ...
                  <tr class="divide-x divide-gray-200 {{ $loop->even ? 'bg-sky-100' : '' }}">
                    ...
                    <td class="px-3 py-2 text-xs text-left whitespace-nowrap select-all"><input class="placeholder-gray-500 w-28 p-1 border border-blue-300 rounded-md" value="{{ URL::to('/') }}/berita/{{ $b->slug }}"></td>
                    <td class="w-20 px-3 py-2 text-xs text-center whitespace-nowrap">
                        <div class="flex flex-row gap-1 ">
                            <a href="{{ URL::to('/') }}/berita/{{ $b->slug }}" target="_blank"><div class="text-xxs text-white bg-sky-600 hover:bg-sky-700 p-1.5 rounded-md"><i class="fas fa-eye"></i></div></a>
                            ...
                            <form action="/admin/berita/{{ $b->slug }}" method="post">
                              @method('delete')
                              @csrf
                              <button id="delete_confirm" class="text-xxs text-white bg-red-600 hover:bg-red-700 p-1.5 rounded-md delete_confirm" data-slug="{{ $b->slug }}"><i class="fas fa-trash-alt"></i></button>
                            </form>
                        </div>
                    </td>
                  </tr>
