...
<div class="w-full h-auto bg-white px-5 my-20 md:mx-8 md:mt-5 md:mb-14">
  <div class="flex flex-row items-center justify-between">
    <div class="md:text-2xl font-semibold">Edit Wartawan</div>
  ...
  <div class="flex flex-col mt-5 bg-gray-100 p-4 h-auto w-full rounded-lg">
    ...
    <form method="post" action="/admin/wartawan/ {{ $wartawan->id_wartawan }}" enctype="multipart/form-data">
    @method('put')
    @csrf
    ...
    <div class="flex flex-col md:flex-row mt-3 items-center">
      ...
      <div class="flex flex-col w-full md:w-5/12">
        <input type="text" id="nama_wartawan" name="nama_wartawan" class="mt-1 md:mt-0 w-full p-2 border border-gray-400 rounded-md" placeholder="Nama" value="{{ old('nama_wartawan', $wartawan->nama_wartawan) }}" autocomplete="off" required>
          @error('nama_wartawan')
    ...
    <div class="flex flex-col md:flex-row mt-3 items-center">
      ...
      <div class="flex flex-col w-full md:w-5/12">
        <select id="status" name="status" class="mt-1 md:mt-0 w-full p-2 border border-gray-400 rounded-md" required>
          @if(old('status', $wartawan->status) == 1)
    ...
    <div class="flex flex-col md:flex-row mt-2 items-center">
      ...
      <div class="flex flex-col w-full md:w-5/12">
        <input type="hidden" name="fotoLama" value="{{ $wartawan->foto }}">
        @if($wartawan->foto)
        <img src="{{ asset('storage/'. $wartawan->foto) }}" class="img-preview mb-2 w-32 max-h-32 rounded-full object-cover overflow-hidden">
        @else
        <img class="img-preview mb-2 w-32 max-h-32 rounded-full object-cover overflow-hidden">
        @endif        <input type="file" id="foto" name="foto" class="mt-1 md:mt-0 w-full p-2 border border-gray-400 rounded-md" onchange="previewGambar()">
        @error('foto')
          <div class="text-red-500 text-xs mt-1 font-semibold">{{ $message }}</div>
        @enderror
      </div>
