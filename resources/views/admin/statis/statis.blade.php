...
<div class="w-full h-auto bg-white px-5 my-20 md:mx-8 md:mt-5 md:mb-14">
  <div class="flex flex-row items-center justify-between">
    <div class="md:text-2xl font-semibold">Management Halaman</div>
  ...
  {{-- main content --}}
    ...
    <div class="mt-5 h-auto w-full">
      <div class="flex flex-row w-full justify-between">
          <a href="/admin/halaman/create"><div class="text-white font-semibold bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg"><i class="fas fa-plus"></i> Tambah Halaman</div></a> 
      </div>
            ...
            <table class="min-w-full">
              <thead class="bg-gradient-to-r from-slate-600 to-gray-600">
                <tr class="divide-x divide-gray-200">
                  ...
                  <th class="px-3 py-2 text-sm text-center uppercase font-bold text-white whitespace-nowrap tracking-wide">Judul</th>
              ...
              <tbody>
                ...
                <tr class="divide-x divide-gray-200 {{ $loop->even ? 'bg-sky-100' : '' }}">
                  ...
                  <td class="px-2 py-2 text-sm text-center whitespace-nowrap md:whitespace-normal">{{ $h->judul }}</td>
                  ...
                  <td class="px-2 py-2 text-sm text-center whitespace-nowrap">{{ $h->urutan }}</td>             
                  <td class="px-3 py-2 text-sm text-center w-24 whitespace-nowrap">
                      <div class="flex flex-row gap-2 ">
                          <a href="/admin/halaman/{{ $h->slug }}/edit"><div class="text-xxs font-semibold text-white bg-amber-500 hover:bg-amber-600 py-2 px-3 rounded-md"><i class="fas fa-pen-alt"></i> Edit</div></a>
                          <form action="/admin/halaman/{{ $h->slug }}" method="post">
                            @method('delete')
                            @csrf
                            <button class="text-xxs font-semibold text-white bg-red-600 hover:bg-red-700 py-2 px-3 rounded-md delete_confirm" data-slug="{{ $h->slug }}"><i class="fas fa-trash-alt"></i> Hapus</button>
                          </form>
                      </div>
                  </td>
                </tr>
